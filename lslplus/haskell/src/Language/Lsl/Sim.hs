...
processAvatarInputEvent k inputEvent = runAndLogIfErr "problem processing event for avatar" () $ do
        av <- getM $ wav k
        let avInfo = [ IVal lslPlusAvatarKey, KVal (getI avatarKey av), 
                       IVal lslPlusAvatarPos, ((vec2VVal . getI avatarPosition) av),
                       IVal lslPlusAvatarRot, ((rot2RVal . getI avatarRotation) av),
                       IVal lslPlusAvatarName, SVal (getI avatarName av) ]
