 ...
 *     rgreayer - hacked to include in LSLForge plug-in without dependencies
 *     on internal (non-API) bits of core plug-ins.
 *******************************************************************************/
...
public class ResourceAndContainerGroup implements Listener {
    ...
    private String resourceType = "resource"; //$NON-NLS-1$
    ...
    protected boolean validateContainer() {
        ...
        if (path == null) {
            ...
            problemMessage = Messages.getString("ResourceAndContainerGroup.NO_FOLDER_SPECIFIED"); //$NON-NLS-1$
            return false;
        }
        ...
        if (projectName == null
            ...
            problemMessage = Messages.getString("ResourceAndContainerGroup.THE_SPECIFIED_PROJECT_DOES_NOT_EXIST"); //$NON-NLS-1$
            return false;
        }
        ...
        while (path.segmentCount() > 1) {
        	if (root.getFile(path).exists()) {
        		...
        		problemMessage = NLS.bind(Messages.getString("ResourceAndContainerGroup.ALREADY_EXISTS"), path.makeRelative()); //$NON-NLS-1$
        		return false;
        	}
     ...
     * @return whether or not all controls are valid
     */
    ...
    protected boolean validateFullResourcePath(IPath resourcePath) {
        ...
        if (!allowExistingResources
            ...
            problemMessage = NLS.bind(Messages.getString("ResourceAndContainerGroup.ALREADY_EXISTS_1"), getResource()); //$NON-NLS-1$
            return false;
        }
        return true;
    }
    ...
    protected boolean validateResourceName() {
        ...
        if (resourceName.length() == 0) {
            ...
            problemMessage = NLS.bind(Messages.getString("ResourceAndContainerGroup.NAME_IS_EMPTY"), resourceType); //$NON-NLS-1$
            return false;
        }

        if (!Path.ROOT.isValidPath(resourceName)) {
            problemType = PROBLEM_NAME_INVALID;
            problemMessage = NLS.bind(Messages.getString("ResourceAndContainerGroup.NOT_A_VALID_FILE_NAME"), resourceName); //$NON-NLS-1$
            return false;
        }
        return true;
    }
