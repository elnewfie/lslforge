...
funcDescriptions = [
    ...
    ("llAgentInExperience",(["agent"],"Determines whether or not the specified agent (in the same region) is in the script's experience.<br/>Returns TRUE if the agent is in the experience and the experience can run in the current region.\n")),
    ...
    ("llCreateKeyValue",(["k","v"],"*** Experience Persistent Storage ***<br/><br/>Start an asynchronous transaction to create a key-value pair associated with the given experience key using the given key and value. Returns a handle (a key) that can be used to identify the corresponding dataserver event to determine if this command succeeded or failed.<br/>string k - The key for the key-value pair<br/>string v - The value for the key-value pair. Maximum 2047 characters, or 4095 if using Mono.<br/>The value for the key-value pair. Maximum 2047 characters, or 4095 if using Mono.<br/>Check wiki for details what dataserver callback returns.\n")),
    ...
    ("llDialog",(["avatar","message","buttons","chat_channel"],"Shows a dialog box on the avatar's screen with the message.<br/>Up to 12 strings in the list form buttons.<br/>If a button is clicked, the name is chatted on chat_channel.\n")),
    ...
    ("llGetAgentInfo",(["id"],"Gets information about agent ID.<br/>Returns AGENT_FLYING, AGENT_ATTACHMENTS, AGENT_SCRIPTED, AGENT_SITTING, AGENT_ON_OBJECT, AGENT_MOUSELOOK, AGENT_AWAY, AGENT_BUSY, AGENT_TYPING, AGENT_CROUCHING, AGENT_ALWAYS_RUN, AGENT_WALKING and/or AGENT_IN_AIR.\n")),
    ...
    ("llGetInventoryNumber",(["type"],"Get the number of items of a given type in the task's inventory.<br/>Valid types: INVENTORY_TEXTURE, INVENTORY_SOUND, INVENTORY_OBJECT, INVENTORY_SCRIPT, INVENTORY_CLOTHING, INVENTORY_BODYPART, INVENTORY_NOTECARD, INVENTORY_LANDMARK, INVENTORY_ALL\n")),
    ...
    ("llGetParcelDetails",(["pos","params"],"Gets the parcel details specified in params for the parcel at pos.<br/>Params is one or more of: PARCEL_DETAILS_NAME, _DESC, _OWNER, _GROUP, _AREA\n")),
    ...
    ("llGetParcelPrimCount",(["pos","category","sim_wide"],"Gets the number of prims on the parcel of the given category.<br/>Categories: PARCEL_COUNT_TOTAL, _OWNER, _GROUP, _OTHER, _SELECTED, _TEMP.\n")),
    ("llGetParcelPrimOwners",(["pos"],"Returns a list of all residents who own objects on the parcel and the number of objects they own.<br/>Requires owner-like permissions for the parcel.\n")),
    ...
    ("llKeysKeyValue",(["first","count"],"*** Experience Persistent Storage ***<br/><br/>Start an asynchronous transaction to request a number of keys. Returns a handle (a key) that can be used to identify the corresponding dataserver event to determine if this command succeeded or failed.<br/>integer first - Zero-based index of the first key to retrieve <br/>integer count - Number of keys to retriever<br/>Check wiki for details and what dataserver callback returns\n")),
    ...
    ("llMapDestination",(["simname","pos","look_at"],"Opens world map centered on region with pos highlighted.<br/>Only works for scripts attached to avatar, or during touch events.\n(NOTE: look_at currently does nothing)\n")),
    ("llMessageLinked",(["linknum","num","str","id"],"Sends num, str, and id to members of the link set (LINK_ROOT sends to root task in a linked set, <br/>LINK_SET sends to all tasks,<br/>LINK_ALL_OTHERS to all other tasks,<br/>LINK_ALL_CHILDREN to all children,<br/>LINK_THIS to the task the script it is in)\n")),
    ...
    ("llParticleSystem",(["rules"],"Creates a particle system based on rules. Empty list removes particle system from object.<br/>List format is [ rule1, data1, rule2, data2 . . . rulen, datan ]\n")),
    ...
    ("llReadKeyValue",(["k"],"*** Experience Persistent Storage ***<br/><br/>Start an asynchronous transaction to read the value associated with the specified key and the specified experience. Returns a handle (a key) that can be used to identify the corresponding dataserver event to determine if this command succeeded or failed and the results.<br/>string k - The key for the key-value pair.<br/>Check wiki for details what dataserver callback returns.\n")),
    ...
    ("llRemoteDataSetRegion",([],"If an object using remote data channels changes regions, you must call this function to reregister the remote data channels.<br/>You do not need to make this call if you don't change regions.\n")),
    ...
    ("llRequestExperiencePermissions",(["agent","name"],"Asks the agent for permission to participate in an experience.<br/>key agent - key of the agent to request permissions from<br/>string name - deprecated, no longer used.<br/>Similar to llRequestPermissions with all permission flags, however the result will flow to experience_permissions or experience_permissions_denied depending on experience grid and agent response.<br/>If no response is given, the request will time out in 5 minutes.\n")),
    ...
    ("llRezAtRoot",(["inventory","pos","vel","rot","param"],"Instantiate owner's inventory object at pos with velocity vel and rotation rot with start parameter param.<br/>The last selected root object's location will be set to pos\n")),
    ...
    ("llSendRemoteData",(["channel","dest","idata","sdata"],"Send an XML-RPC request to dest through channel with payload of channel (in a string), integer idata and string sdata.<br/>A message identifier key is returned.<br/>An XML-RPC reply will trigger a remote_data event and reference the message id.<br/>The message_id is returned.\n")),
    ...
    ("llSetAngularVelocity", (["force", "local"], "Applies rotational velocity to object.<br/>integer local - boolean, if TRUE uses local axis, if FALSE uses region axis\n")),
    ...
    ("llSetCameraParams",(["rules"],"Sets multiple camera parameters at once.<br/>List format is [ rule1, data1, rule2, data2 . . . rulen, datan ]\n")),
    ...
    ("llSetVelocity", (["force", "local"], "Applies velocity to object<br/>integer local - boolean, if TRUE, force is treated as a local directional vector instead of region directional vector\n")),
    ("llShout",(["channel","msg"],"shouts msg on channel\n")),
    ("llSin",(["theta"],"theta in radians\n")),
    ("llSitOnLink",(["agent_id","link"],"*** Experience enabled script only ***<br/><br/>The avatar specified by agent_id is forced to sit on the sit target of the prim indicated by the link parameter. If the specified link is already occupied, the simulator searches down the chain of prims in the link set looking for an available sit target.<br/>Returns 1 is success or one of negative number SIT_* constants if failed\n")),
    ...
    ("llStringTrim",(["src","trim_type"],"Trim leading and/or trailing spaces from a string.<br/>Uses trim_type of STRING_TRIM, STRING_TRIM_HEAD or STRING_TRIM_TAIL.\n")),
    ...
    ("llTransferLindenDollars", (["destination", "amount"], "Transfer amount of L$ money from script owner to destination avatar.<br/>Returns a key used in a matching transaction_result event for the success or failure of the transfer. If the transaction is successful, this key will show in the transaction history.\n")),
    ...
    ("llVolumeDetect",(["detect"],"If detect = TRUE, object becomes phantom but triggers collision_start and collision_end events when other objects start and stop interpenetrating. <br/>Must be applied to the root object.\n")),
